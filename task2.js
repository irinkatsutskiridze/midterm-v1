...
const students = [
  ...
  function getSubjectsStatistics(students) {
    // თქვენი კოდი აქ
    return students.reduce((name, grades) => (math + history + physics) /  3 )
  }
  
  function getHighAchievers(students, gradeThreshold) {
    // თქვენი კოდი აქ
    return students
    .filter((name, average ) => average < gradeThreshold)
  ...
  function getMultiAchivers(students, threshold) {
    // თქვენი კოდი აქ
    return students
    .filter((firstName, grades) => grades > threshold)
    .map((firstName) => threshold)
  };
